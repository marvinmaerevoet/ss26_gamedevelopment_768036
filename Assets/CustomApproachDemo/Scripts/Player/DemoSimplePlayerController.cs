...
namespace CustomApproachDemo.Player
{
    public sealed class DemoSimplePlayerController : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            Vector3 horizontalVelocity = hasMovement ? moveDirection * speed : Vector3.zero;
            Move(horizontalVelocity);
            ReportMovement(horizontalVelocity.magnitude, isRunning);
        }
        ...
        private void ReportMovement(float movementSpeed, bool isRunning)
        {
            if (playerState != null)
            {
